 ...
 *
 * Admin auth (login + password reset) serves staff only and leads into the
 * desktop-only dashboard, so below 1024px it shows the same desktop-only
 * fallback as /admin instead of the login card.
 */
export default function AuthLayout({ children }: { children: ReactNode }) {
  return (
    <>
      ...
      {/* Below 1024px: same desktop-only fallback as /admin */}
